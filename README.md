...
This program has some limitations and may not be able to solve some of the more extensive cases. For instance, in a social golf scenario with 24 participants divided into 8 groups (of 4 players each) the number of rounds/days it can be solved for is supposed to be 10, but this program can only work out 9 of the rounds quickly (in less than a second). Allowed to run for a minute, it will often produce a complete 10 rounds with only one slot in one group unfilled. Left to run longer, it will eventually produce the full solution, but this is an unusually difficult set of parameters to solve for.
